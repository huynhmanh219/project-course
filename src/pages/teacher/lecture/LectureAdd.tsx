...
const LectureAdd: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Creating lecture:', createData);
      
      const createdLecture = await simpleLectureService.createLecture(createData);
      ...
      if (videoFile) {
        ...
        try {
          ...
          setError(`Bài giảng đã được tạo nhưng upload video thất bại: ${uploadError.message}`);
          setUploading(false);
          return;
        }
      ...
      setError(error.message || 'Lỗi khi tạo bài giảng');
    } finally {
      setSaving(false);
      setUploading(false);
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <nav className="text-sm text-gray-500 mb-2">
              ...
              <span className="text-gray-800 font-medium">Tạo bài giảng mới</span>
            </nav>
            <h1 className="text-2xl font-bold text-gray-800">Tạo bài giảng mới</h1>
          </div>
        </div>
